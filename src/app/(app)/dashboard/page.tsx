...
export default function UserDashboard() {
  ...
  //Purpose: Fetches the "accept messages" state from the server and updates the form.
  ...
  const fetchMessages=useCallback( async(refresh:boolean =false)=>{
    //refresh -Purpose: Indicates whether the function call is specifically intended to refresh (re-fetch) the latest messages.
    setIsLoading(true)
    setIsSwitchLoading(false)
    try {
  ...
  // Fetch initial state from the serve
  useEffect(()=>{
   ...
   const handleSwitchChange= async ()=>{
      try{
        ...
        toast({
          title:'Error',
          description:
          axiosError.response?.data.message || 'Faled to update the message settings',
          variant:'destructive'
        });
      }
   };
